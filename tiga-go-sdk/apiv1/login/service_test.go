package loginv1_test

import (
	"context"
	commonv1 "github.com/absurdlab/tiga-sdk/tiga-go-sdk/apiv1/common"
	loginv1 "github.com/absurdlab/tiga-sdk/tiga-go-sdk/apiv1/login"
...
func (s *defaultServiceTestSuite) TestGetAuthentication() {
	service := newService()

	cases := []struct {
		name      string
		sessionId string
		assert    func(t *testing.T, req *loginv1.AuthenticationRequest, err error)
	}{
		{
			name:      "ok",
			sessionId: "ok",
	...
	for _, c := range cases {
		s.T().Run(c.name, func(t *testing.T) {
			resp, err := service.GetAuthentication(
				context.Background(),
				loginv1.Routes.GetAuthenticationRequestEndpoint(tigaBaseURL, c.sessionId),
			)
			c.assert(t, resp, err)
		})
	}
...
func newService() loginv1.Service {
	return loginv1.NewService(&clientcredentials.Config{
		ClientID:     "test",
		ClientSecret: "s3cret",
		TokenURL:     tigaBaseURL + "/oauth/token",
		Scopes:       []string{"tiga:login"},
	})
}
